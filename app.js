...
io.sockets.on('connection', function (socket) {
  ...
  Chats.find({},function(error, rooms){
    ...
    socket.on('adduser', function(data){
      ...
      if(rooms[0]){
        ...
        socket.broadcast.to(rooms[0]._id).emit('updatechat', 'SERVER', data.username + ' has connected to this room');
        socket.emit('updaterooms', rooms, rooms[0]._id);

        Messages.find({ chatId: rooms[0]._id },function(error, messages){
          if(error){return res.status(422).send("error when saving message")};
  ...
  socket.on('sendchat', function (data) {
    ...
    var newMessage = new Messages({
      ...
      if(error){return res.status(422).send("error when saving message")};
      io.sockets.in(socket.room).emit('updatechat', socket.username, data.messageContent);
    });
  });
  ...
  socket.on('switchRoom', function(newroom){
    Chats.find({},function(error, rooms){
      ...
      Messages.find({ chatId: newroom },function(error, messages){
        if(error){return res.status(422).send("error when fetching old messages")};
          socket.emit( 'initiatechat', messages );
      });
    });
  });
